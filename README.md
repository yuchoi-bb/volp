...
| | `volp-<버전>.apk` (안전판) | `volp-full-<버전>.apk` |
| --- | --- | --- |
...
서명과 패키지 이름이 같아 서로 덮어써도 **기록은 그대로 이어진다**. 평소에는 안전판을 쓰고,
카드 문자를 저절로 모으고 싶을 때만 PC를 꺼내 자동 수집판으로 갈아타면 된다.

앱 안에서 새 버전을 확인할 때는 **지금 깔린 것과 같은 갈래**를 골라 온다. 안전판 APK가 표시
없는 이름을 쓰는 것도 이 때문이다. 갈래를 모르던 예전 버전이 첫 APK를 집어 가더라도 안전판이
잡힌다.

```bash
adb install -r volp-full-1.0.19.apk
```
